 ...
 * operation optimized and path compression. Ack: Kevin Wayne.
 *
 * @author   Dean Hendrix (dh@auburn.edu)
 * @version  2019-04-19
 *
 */
public class PathCompressionDS implements DisjointSet {
    ...
    public String toString() {
        ...
        /**
         * Calling find() here will alter the structure, so only call root.
         */
        HashMap<Integer, List<Integer>> map = new HashMap<>();
        for (int i = 0; i < id.length; i++) {
            List<Integer> vertices = map.get(root(i));
            if (vertices == null) {
                vertices = new ArrayList<Integer>();
            }
            vertices.add(i);
            map.put(root(i), vertices);
        }
